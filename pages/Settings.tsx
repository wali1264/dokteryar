...
const LETTER_DIMS = { w: 816, h: 1056 };
...
const Settings: React.FC = () => {
  ...
  const getCanvasDimensions = () => {
    switch (paperSettings.paperSize) {
      case 'A5': return A5_DIMS;
      case 'Letter': return LETTER_DIMS;
      default: return A4_DIMS;
    }
  };
            ...
            <div className="space-y-6">
               <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center border-b pb-4 gap-4">
                  ...
                  <div className="flex flex-wrap gap-2 w-full lg:w-auto">
                     <div className="flex items-center gap-1 bg-gray-100 p-1 rounded-lg">
                        ...
                        <button onClick={() => setPaperSettings(p => ({ ...p, paperSize: 'Letter' }))} className={`px-3 py-1 rounded text-sm font-bold transition-all ${paperSettings.paperSize === 'Letter' ? 'bg-white shadow text-blue-600' : 'text-gray-500'}`}>Letter</button>
                     ...
                     <div className="relative"><input type="file" accept="image/*" className="absolute inset-0 opacity-0 cursor-pointer" onChange={e => handleFileChange(e, val => setPaperSettings(p => ({ ...p, backgroundImage: val })))} /><button className="bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 hover:bg-gray-50"><Upload size={16} /> آپلود عکس سربرگ</button></div>
                     <button onClick={() => setPaperSettings(p => ({ ...p, backgroundImage: '' }))} className="text-red-500 text-xs px-2 hover:bg-red-50 rounded">حذف عکس</button>
                     <button onClick={handleSavePaper} className="bg-emerald-600 text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 shadow hover:bg-emerald-700 ml-auto lg:ml-0"><Save size={16} /> ذخیره نهایی</button>
                  </div>
               </div>
               <div className="flex flex-col lg:flex-row gap-6 lg:h-[700px]">
                  <div className="flex-1 bg-gray-200 rounded-2xl overflow-auto p-4 lg:p-8 flex justify-center relative shadow-inner min-h-[400px]">
                     <div ref={canvasRef} style={{ width: dims.w + 'px', height: dims.h + 'px', backgroundImage: paperSettings.backgroundImage ? `url(${paperSettings.backgroundImage})` : 'none', backgroundSize: '100% 100%' }} className="bg-white relative transition-all duration-300 transform scale-[0.6] origin-top lg:scale-100 lg:origin-center">
                        ...
                        {paperSettings.elements.filter(el => el.visible).map(el => (
                           <div key={el.id} onMouseDown={(e) => handleMouseDown(e, el.id)} onTouchStart={(e) => handleTouchStart(e, el.id)} style={{ position: 'absolute', left: el.x + 'px', top: el.y + 'px', width: el.width + 'px', fontSize: el.fontSize + 'pt', transform: `rotate(${el.rotation}deg)`, cursor: isDragging ? 'grabbing' : 'grab', border: selectedElementId === el.id ? '2px solid #3b82f6' : 'none', backgroundColor: selectedElementId === el.id ? 'rgba(59, 130, 246, 0.1)' : 'transparent', zIndex: selectedElementId === el.id ? 10 : 1, textAlign: el.align || 'right' }} className="group select-none touch-none overflow-visible">
                               {el.label}
                  ...
                  <div className="w-full lg:w-80 bg-white border border-gray-200 rounded-2xl p-6 lg:overflow-y-auto">
                     <h4 className="font-bold text-gray-800 mb-4 border-b pb-2 flex items-center gap-2"><Layers size={18} className="text-gray-500" /> لیست المان‌ها</h4>
                     <div className="space-y-2 mb-6 max-h-48 overflow-y-auto custom-scrollbar">{paperSettings.elements.map(el => (<div key={el.id} className={`flex items-center justify-between p-2 rounded-lg cursor-pointer transition-colors ${selectedElementId === el.id ? 'bg-blue-50' : 'bg-gray-50 hover:bg-gray-100'}`} onClick={() => setSelectedElementId(el.id)}><div className="flex items-center gap-2"><input type="checkbox" checked={el.visible} onChange={(e) => { e.stopPropagation(); setPaperSettings(prev => ({ ...prev, elements: prev.elements.map(item => item.id === el.id ? { ...item, visible: e.target.checked } : item) })); }} /><span className={`text-sm ${selectedElementId === el.id ? 'font-bold text-blue-600' : 'text-gray-700'}`}>{el.label}</span></div></div>))}</div>
                     {selectedEl ? (<div className="bg-blue-50 p-4 rounded-xl border border-blue-100 space-y-4 animate-fade-in"><h5 className="font-bold text-blue-900 text-sm flex items-center gap-2"><Type size={14} /> تنظیمات: {selectedEl.label}</h5><div className="grid grid-cols-2 gap-2"><div className="space-y-1"><label className="text-xs text-gray-500">سایز قلم</label><input type="number" className="w-full p-2 rounded border border-gray-200 text-sm" value={selectedEl.fontSize} onChange={e => updateSelectedElement('fontSize', parseInt(e.target.value))} /></div><div className="space-y-1"><label className="text-xs text-gray-500">عرض باکس</label><input type="number" className="w-full p-2 rounded border border-gray-200 text-sm" value={selectedEl.width} onChange={e => updateSelectedElement('width', parseInt(e.target.value))} /></div></div><div className="space-y-1"><label className="text-xs text-gray-500 flex items-center gap-1"><RotateCw size={10} /> چرخش (درجه)</label><input type="range" min="-90" max="90" className="w-full" value={selectedEl.rotation} onChange={e => updateSelectedElement('rotation', parseInt(e.target.value))} /></div><div className="flex justify-between bg-white p-2 rounded border border-gray-200"><button onClick={() => updateSelectedElement('align', 'right')} className={`p-1 flex-1 rounded text-xs font-bold ${selectedEl.align === 'right' ? 'bg-blue-600 text-white' : ''}`}>راست</button><button onClick={() => updateSelectedElement('align', 'center')} className={`p-1 flex-1 rounded text-xs font-bold ${selectedEl.align === 'center' ? 'bg-blue-600 text-white' : ''}`}>وسط</button><button onClick={() => updateSelectedElement('align', 'left')} className={`p-1 flex-1 rounded text-xs font-bold ${selectedEl.align === 'left' ? 'bg-blue-600 text-white' : ''}`}>چپ</button></div></div>) : (<p className="text-sm text-gray-400 text-center py-4 bg-gray-50 rounded-xl font-bold">جهت تنظیم، یک المان را از لیست بالا انتخاب کنید</p>)}
                     <div className="mt-6 pt-6 border-t border-gray-200"><label className="flex items-start gap-3 cursor-pointer"><div className="relative flex items-center"><input type="checkbox" className="sr-only peer" checked={paperSettings.printBackground} onChange={e => setPaperSettings(p => ({ ...p, printBackground: e.target.checked }))} /><div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white peer-checked:bg-blue-600"></div></div><div><span className="text-sm font-bold text-gray-700 block">چاپ عکس سربرگ</span><span className="text-[10px] text-gray-500 block leading-relaxed">اگر سربرگ شما از قبل چاپ شده است، این گزینه را غیرفعال کنید.</span></div></label></div>
                  </div>
               </div>
            </div>
          )}
