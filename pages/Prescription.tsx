...
const Prescription: React.FC<PrescriptionProps> = ({ initialRecord }) => {
  ...
  const [isNewPatientPadMode, setIsNewPatientPadMode] = useState(true); 
  ...
  const currentTempSessionId = useRef<string>(crypto.randomUUID());
  ...
  useEffect(() => {
    if (isNewPatientPadMode && selectedPatient?.name) {
       const exists = allPatients.some(p => p.name.trim() === selectedPatient.name.trim());
       setDuplicateWarning(exists);
    } else {
       setDuplicateWarning(false);
    }
  }, [selectedPatient?.name, allPatients, isNewPatientPadMode]);
  ...
  const handleSelectPatient = (patient: PatientRecord) => {
    // Check if this patient is already in the queue
    const sessionInQueue = activeSessions.find(s => s.id === patient.id || (s.patient && s.patient.id === patient.id));
    
    if (sessionInQueue) {
        loadSessionFromQueue(sessionInQueue);
        return;
    }

    if (selectedPatient?.name || items.some(it => it.drug) || diagnosis || chiefComplaint) {
        saveCurrentToQueue();
    }

    setSelectedPatient(patient);
    setIsExpressMode(patient?.id?.startsWith('guest_') || false);
    setViewMode('editor');
    setVitals({
    ...
    if (patient?.id && !patient.id.startsWith('guest_')) {
    ...
    currentTempSessionId.current = patient.id;
  };
  ...
  const discardDraft = () => {
    if (selectedPatient?.id) localStorage.removeItem(`tabib_draft_${selectedPatient.id}`);
    setShowDraftBanner(false);
    setActiveDraft(null);
  };
  ...
  const startDigitalPad = () => {
    ...
    const paperWidth = settings.paperSize === 'A4' ? A4_DIMS.w : A5_DIMS.w;
    const initialX = (window.innerWidth / 2) - (paperWidth / 2) - 140; 
    setPadToolbarPos({ x: Math.max(initialX, 20), y: (window.innerHeight / 2) - 250 });
    ...
    finalCanvas.toBlob(async (blob) => {
      if (blob) {
        ...
        try {
          ...
          const guestRecord: PatientRecord = {
            id: (res.patientName && res.patientName !== 'بیمار مهمان') ? `guest_${Date.now()}` : currentTempSessionId.current,
          ...
          // Successful analysis logic
          setSelectedPatient(guestRecord);
  ...
  const resetPadForNewPatient = () => {
    ...
    currentTempSessionId.current = crypto.randomUUID();
    ...
    setIsNewPatientPadMode(true);
    setDuplicateWarning(false);
  };

  const saveCurrentToQueue = () => {
    const name = selectedPatient?.name || "بیمار بی‌نام";
    const sId = selectedPatient?.id || currentTempSessionId.current;
    
    if (name === 'بیمار بی‌نام' && !diagnosis && !chiefComplaint && !items.some(i => i.drug)) return;

    const session = {
        id: sId,
        patient: selectedPatient ? { ...selectedPatient } : { id: sId, name: name, age: '', gender: 'male', vitals: { ...vitals } },
        vitals: { ...vitals },
        diagnosis,
        chiefComplaint,
  ...
  const loadSessionFromQueue = (session: any) => {
    if (selectedPatient?.name || items.some(it => it.drug) || diagnosis || chiefComplaint) {
        saveCurrentToQueue();
    }
    
    const isGuest = session.patient?.id?.startsWith('guest_') || (session.id && session.id.length > 20);
    
    setSelectedPatient(isGuest ? { ...session.patient, name: (session.patient?.name === 'بیمار بی‌نام' || !session.patient?.name) ? '' : session.patient.name } : session.patient);
    setVitals({ ...session.vitals });
    setDiagnosis(session.diagnosis || '');
    setChiefComplaint(session.chiefComplaint || '');
    setItems([...(session.items || [])]);
    setIsExpressMode(isGuest);
    setShowSessionQueue(false);
    currentTempSessionId.current = session.id;
    setIsNewPatientPadMode(isGuest && (!session.patient || !session.patient.name || session.patient.name === 'بیمار بی‌نام'));
    setViewMode('editor'); 
  };

  const removeSessionFromQueue = (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    if(confirm('آیا از حذف این ویزیت ناتمام از لیست انتظار اطمینان دارید؟')) {
  ...
  const saveToPatientRecord = async () => {
    ...
    if (isNewPatientPadMode || !selectedPatient?.id || selectedPatient.id.length > 30) {
        ...
        if (existing.length > 0 && isNewPatientPadMode) {
            alert(`خطا: پرونده‌ای با نام «${selectedPatient.name}» قبلاً ثبت شده است.`);
            return;
        }
  ...
  const updateItem = (index: number, field: keyof PrescriptionItem, value: string) => {
    ...
    const finalValue = (field === 'drug' || field === 'instruction') ? toTitleCase(value) : value;
  ...
  const selectSuggestedDrug = (drugName: string) => {
    ...
    focusField(`item_dosage_${activeItemIndex}`);
  };
  ...
  const selectSuggestedDosage = (dosage: string) => {
    ...
    setItems(newItems); 
    setSuggestionType(null);
    focusField(`item_instruction_${activeItemIndex}`);
  };
  ...
  const selectSuggestedInstruction = (instruction: string) => {
    ...
    setTimeout(() => focusField(`item_drug_${activeItemIndex + 1}`), 50);
  };
  ...
  const handleEnterKey = (e: React.KeyboardEvent, currentId: string) => {
      if (e.key === 'Enter') {
          e.preventDefault();
          const sequence = [
              'patientName', 'age', 'chiefComplaint', 
              'vital_bp', 'vital_hr', 'vital_rr', 'vital_temp', 'vital_o2', 'vital_bs', 'vital_weight',
              'items'
          ];
          
          if (currentId.startsWith('item_')) {
              const parts = currentId.split('_');
              const field = parts[1];
              const idx = parseInt(parts[2]);
              if (field === 'drug') focusField(`item_dosage_${idx}`);
              else if (field === 'dosage') focusField(`item_instruction_${idx}`);
              else if (field === 'instruction') {
                  if (idx < items.length - 1) focusField(`item_drug_${idx + 1}`);
                  else {
                      addItem();
                      setTimeout(() => focusField(`item_drug_${idx + 1}`), 50);
                  }
              }
              return;
          }

          const curIndex = sequence.indexOf(currentId);
          if (curIndex > -1 && curIndex < sequence.length - 1) {
              const nextId = sequence[curIndex + 1];
              if (nextId === 'items') {
                  focusField('item_drug_0');
              } else {
                  if (currentId.startsWith('vital_') && !(e.target as any).value) {
                      focusField('item_drug_0');
                  } else {
                      focusField(nextId);
                  }
      ...
      {viewMode === 'landing' ? renderLanding() : (
        ...
        <div className="hidden lg:block min-h-screen">
           ...
           <div className="flex gap-6 items-start">
              ...
              <div className="flex-1 flex flex-col gap-6"><div className={`p-4 rounded-[2rem] border transition-all duration-500 shadow-sm ${isRecordingScribe ? 'bg-purple-50/50 scribe-glow border-purple-200' : 'bg-white border-gray-100'}`}><div className="flex items-center justify-between mb-2 px-4"><div className="flex items-center gap-3"><label className="flex items-center gap-2 text-indigo-800 font-black text-xs uppercase tracking-widest"><Activity size={16} /> <span>تشخیص نهایی پزشک متخصص</span></label><div className="flex gap-2"><button onClick={() => setShowComplaintModal(true)} className={`flex items-center gap-2 px-3 py-1.5 rounded-xl text-[10px] font-black transition-all ${chiefComplaint ? 'bg-indigo-600 text-white shadow-lg' : 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100'}`}><ListChecks size={14} />{chiefComplaint ? 'شکایات ثبت شد' : 'شکایات بیمار'}</button><button onClick={() => setShowPreferenceModal(true)} className="flex items-center gap-2 px-3 py-1.5 rounded-xl text-[10px] font-black bg-indigo-50 text-indigo-600 hover:bg-indigo-100 transition-all"><RotateCw size={14} />ترجیحات</button></div></div>{isRecordingScribe && (<div className="flex gap-1 items-end h-4">{[...Array(10)].map((_, i) => <div key={i} className="waveform-bar" style={{ animationDelay: `${i * 0.05}s` }}></div>)}</div>)}</div><input className={`w-full p-4 bg-gray-50/50 border border-transparent focus:border-indigo-100 focus:bg-white rounded-2xl text-xl font-black text-gray-800 outline-none transition-all ${isRecordingScribe ? 'placeholder:italic' : ''}`} placeholder={isRecordingScribe ? "در حال استخراج تشخیص..." : "Working Diagnosis (Differential Diagnosis)..."} value={diagnosis} onChange={e => setDiagnosis(toTitleCase(e.target.value))} /></div><div className={`flex-1 bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100 min-h-[400px] flex flex-col transition-all duration-500 ${isRecordingScribe ? 'scribe-glow' : ''}`}><table className="w-full text-right border-separate border-spacing-y-4"><thead><tr className="border-b border-gray-50"><th className="pb-4 text-[11px] font-black text-gray-400 uppercase w-10 text-center">#</th><th className="pb-4 text-[11px] font-black text-gray-400 uppercase w-3/5">نام دارو و شکل دارویی (Drug Name, Strength, Form)</th><th className="pb-4 text-[11px] font-black text-gray-400 uppercase w-32 text-center">تعداد (Qty)</th><th className="pb-4 text-[11px] font-black text-gray-400 uppercase">دستور مصرف (Sig)</th><th className="pb-4 w-12"></th></tr></thead><tbody className="divide-y divide-gray-50">{items.map((item, idx) => (
                <tr key={idx} className="group hover:bg-indigo-50/20 transition-all rounded-2xl overflow-hidden">
                  ...
                  <td className="py-2 px-2 relative">
                    <input 
                      ...
                      onBlur={() => { setTimeout(() => { if (activeTypingFieldId !== `item_drug_${idx}`) setSuggestionType(prev => prev === 'drug' ? null : prev); }, 400); }} 
                      onChange={e => updateItem(idx, 'drug', e.target.value)} 
                      onKeyDown={e => handleEnterKey(e, `item_drug_${idx}`)}
                  ...
                  <td className="py-2 px-2 relative">
                    <input 
                      ...
                      onBlur={() => { setTimeout(() => { if (activeTypingFieldId !== `item_dosage_${idx}`) setSuggestionType(prev => prev === 'dosage' ? null : prev); }, 400); }} 
                      onChange={e => updateItem(idx, 'dosage', e.target.value)} 
                      onKeyDown={e => handleEnterKey(e, `item_dosage_${idx}`)}
                      placeholder="N=30" 
                    />
                    {suggestionType === 'dosage' && activeItemIndex === idx && (
                      <div className="absolute top-full right-0 left-0 bg-white shadow-2xl rounded-2xl border border-gray-100 z-[9999] overflow-hidden mt-2 p-3 flex flex-col gap-1 animate-slide-up min-w-[120px]">
                        <p className="text-[9px] font-black text-gray-300 uppercase px-2 mb-1">تعداد</p>
                  ...
                  <td className="py-2 px-2 relative">
                    <input 
                      ...
                      onBlur={() => { setTimeout(() => { if (activeTypingFieldId !== `item_instruction_${idx}`) setSuggestionType(prev => prev === 'instruction' ? null : prev); }, 400); }} 
                      onChange={e => updateItem(idx, 'instruction', e.target.value)} 
                      onKeyDown={e => handleEnterKey(e, `item_instruction_${idx}`)}
                      placeholder="N=30" 
                    />
                    {suggestionType === 'instruction' && activeItemIndex === idx && (
                      <div className="absolute top-full right-0 left-0 bg-white shadow-2xl rounded-2xl border border-gray-100 z-[9999] overflow-hidden mt-2 p-3 flex flex-col gap-1 animate-slide-up min-w-[250px]">
                        <p className="text-[9px] font-black text-gray-300 uppercase px-2 mb-1">دستور مصرف</p>
      ...
      {showDigitalPad && (
        <div className="fixed inset-0 z-[420] bg-gray-950 flex flex-col overflow-hidden animate-fade-in"
           ...
           <div 
              ...
              className={`fixed satellite-bar p-2 flex items-center gap-3 touch-none shadow-[0_0_50px_rgba(0,0,0,0.3)] ${padRotation % 180 === 0 ? 'flex-col' : 'flex-row'}`}
           >
              <div className={`flex items-center gap-2 ${padRotation % 180 === 0 ? 'flex-col' : 'flex-row'}`}>
                ...
                <div className={`flex gap-1.5 p-1 bg-gray-50 rounded-2xl shadow-inner ${padRotation % 180 === 0 ? 'flex-col' : 'flex-row'}`}>
                   <button onPointerDown={(e) => { e.stopPropagation(); undoPad(); }} disabled={padHistory.length <= 1} className="p-2 lg:p-3 text-gray-400 hover:text-indigo-600 disabled:opacity-20 transition-all" title="بازگشت (Undo)">
                      <RotateCcw size={18} style={{ transform: `rotate(${-padRotation}deg)` }} />
                   </button>
                   <button onPointerDown={(e) => { e.stopPropagation(); clearPad(); }} className="p-2 lg:p-3 text-red-300 hover:text-red-600 transition-all" title="پاک‌سازی کل صفحه">
                ...
                <div className={`flex gap-1.5 p-1 bg-gray-50 rounded-2xl shadow-inner relative ${padRotation % 180 === 0 ? 'flex-col' : 'flex-row'}`}>
                            ...
                            {padSearchTerm && padFilteredPatients.length > 0 && (
                                <div className="pad-search-results">
                                    {padFilteredPatients.map(p => (
                                        <button key={p.id} onClick={() => handleSelectPatient(p)} className="w-full text-right p-4 border-b border-gray-50 last:border-0 flex justify-between items-center transition-colors">
                                            <div className="flex items-center gap-3"><div className="w-8 h-8 bg-indigo-50 rounded-full flex items-center justify-center text-indigo-600 font-bold text-xs">{p.displayId}</div><span className="font-bold text-sm text-gray-700">{p.name}</span></div>
                                            <span className="text-[10px] bg-gray-100 px-2 py-0.5 rounded text-gray-400">#{p.displayId}</span>
                                        </button>
                                    ))}
                                </div>
                            )}
                            <div className="pad-search-glass"><input autoFocus className="w-full bg-transparent outline-none font-bold text-gray-700 text-sm placeholder-indigo-300" placeholder="نام یا کد (001)..." value={padSearchTerm} onChange={e => setPadSearchTerm(e.target.value)} /></div>
                        </div>
                    )}
                    <button onPointerDown={(e) => { e.stopPropagation(); setIsNewPatientPadMode(!isNewPatientPadMode); }} className={`p-2 lg:p-3 rounded-xl transition-all ${isNewPatientPadMode ? 'bg-blue-600 text-white shadow-md' : 'text-gray-400 hover:text-indigo-600'}`} title={isNewPatientPadMode ? "ثبت فعال" : "ثبت غیرفعال"}>
                ...
                <div className={`flex gap-1.5 p-1 bg-gray-50 rounded-2xl shadow-inner ${padRotation % 180 === 0 ? 'flex-col' : 'flex-row'}`}>
                    <button onPointerDown={(e) => { e.stopPropagation(); setPadTool(prev => prev === 'pen' ? 'idle' : 'pen'); triggerSettingsDisplay(); }} className={`p-2 lg:p-3 rounded-xl transition-all ${padTool === 'pen' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-400'}`}><PenTool size={18} style={{ transform: `rotate(${-padRotation}deg)` }} /></button>
                    <button onPointerDown={(e) => { e.stopPropagation(); setPadTool(prev => prev === 'eraser' ? 'idle' : 'eraser'); triggerSettingsDisplay(); }} className={`p-2 lg:p-3 rounded-xl transition-all ${padTool === 'eraser' ? 'bg-white text-red-500 shadow-sm' : 'text-gray-400'}`}><Eraser size={18} style={{ transform: `rotate(${-padRotation}deg)` }} /></button>
                    <button onPointerDown={(e) => { e.stopPropagation(); setPadTool(prev => prev === 'type' ? 'idle' : 'type'); triggerSettingsDisplay(); }} className={`p-2 lg:p-3 rounded-xl transition-all ${padTool === 'type' ? 'tool-active-glow' : 'text-gray-400'}`}><Type size={18} style={{ transform: `rotate(${-padRotation}deg)` }} /></button>
                </div>
              ...
              <div className={`${padRotation % 180 === 0 ? 'w-full h-[1px]' : 'w-[1px] h-full'} bg-gray-200 mx-1`}></div>

              <div className={`flex items-center gap-2 ${padRotation % 180 === 0 ? 'flex-col' : 'flex-row'}`}>
                 <button onPointerDown={(e) => { e.stopPropagation(); resetPadForNewPatient(); }} className="p-2 lg:p-3 text-emerald-600 bg-emerald-50 hover:bg-emerald-100 rounded-2xl transition-all" title="شروع نشست جدید (بیمار جدید)">
                    <RefreshCw size={18} style={{ transform: `rotate(${-padRotation}deg)` }} />
                 </button>
                 <button onPointerDown={(e) => { e.stopPropagation(); setShowSessionQueue(!showSessionQueue); }} className={`p-2 lg:p-3 rounded-2xl transition-all relative ${showSessionQueue ? 'bg-indigo-600 text-white shadow-md' : 'text-indigo-600 bg-indigo-50 hover:bg-indigo-100'}`} title="صف انتظار هوشمند">
                    <Layers size={18} style={{ transform: `rotate(${-padRotation}deg)` }} />
                    {activeSessions.length > 0 && <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-600 text-white text-[10px] font-black flex items-center justify-center rounded-full animate-bounce shadow-lg border border-white">
                      {activeSessions.length}
                    </span>}
                 </button>
                 
                 {showSessionQueue && (
                    <div className={`absolute ${padRotation % 180 === 0 ? 'left-full top-0 ml-3' : 'top-full left-0 mt-3'} glass-queue z-[500] animate-slide-up pointer-events-auto overflow-hidden`}>
                        <div className="p-3 bg-indigo-600 text-white text-[9px] font-black uppercase tracking-widest text-center shadow-md">صف بیماران معلق</div>
                        <div className="max-h-[500px] overflow-y-auto custom-scrollbar">
                           {activeSessions.length === 0 ? <p className="p-6 text-center text-[10px] text-gray-500 font-black">نشستی یافت نشد</p> : activeSessions.map(s => (
                              <div key={s.id} className="flex items-center justify-between border-b border-gray-100 last:border-0 hover:bg-white/50 transition-all">
                                 <button onClick={() => loadSessionFromQueue(s)} className={`flex-1 text-right p-4 flex flex-col gap-1 transition-all ${selectedPatient?.id === s.id || currentTempSessionId.current === s.id ? 'bg-indigo-600 text-white' : ''}`}>
                                    <span className={`font-black text-[12px] truncate ${selectedPatient?.id === s.id || currentTempSessionId.current === s.id ? 'text-white' : 'text-indigo-900'}`}>{s.patient?.name || 'بیمار بی‌نام'}</span>
                                    <p className={`text-[9px] truncate ${selectedPatient?.id === s.id || currentTempSessionId.current === s.id ? 'text-white/70' : 'text-gray-500'}`}>{s.diagnosis || 'داده‌های ناتمام...'}</p>
                                 </button>
                                 <button onClick={(e) => removeSessionFromQueue(e, s.id)} className="p-4 text-red-300 hover:text-red-500 transition-colors" title="حذف از لیست">
                                    <Trash2 size={16} />
                                 </button>
                              </div>
                           ))}
              ...
              {showPadSettings && (padTool === 'pen' || padTool === 'eraser') && (
                 <div className={`absolute ${padRotation % 180 === 0 ? 'left-full top-1/2 -translate-y-1/2 ml-3' : 'top-full left-1/2 -translate-x-1/2 mt-3'} animate-slide-up z-[460]`}>
                 ...
                 {padTool === 'type' && (
                    <div className="rx-typing-overlay" style={{ width: `${paperDims.w}px`, height: `${paperDims.h}px` }}>
                       {settings.elements.filter(el => el.visible).map(el => {
                          if (el.id === 'patientId') return null; 
                          ...
                          if (isItems) {
                                         ...
                                         <div key={idx} className="rx-item-row group relative overflow-visible bg-white/50 border border-transparent hover:border-indigo-100 hover:bg-white transition-all shadow-sm">
                                            <div className="flex-[5] relative overflow-visible">
                                               <input 
                                                   ...
                                                   onChange={e => updateItem(idx, 'drug', e.target.value)} 
                                                   onFocus={() => { setActiveItemIndex(idx); setSuggestionType('drug'); setSearchQuery(item.drug); setActiveTypingFieldId(`item_drug_${idx}`); }}
                                                   onBlur={() => { setTimeout(() => { if (activeTypingFieldId === `item_drug_${idx}`) { setSuggestionType(null); setActiveTypingFieldId(null); } }, 400); }}
                                                   onKeyDown={e => handleEnterKey(e, `item_drug_${idx}`)}
                                               />
                                               {suggestionType === 'drug' && activeItemIndex === idx && getDrugSuggestions().length > 0 && (
                                            ...
                                            <div className="flex-[1.5] relative overflow-visible">
                                               <input 
                                                   ...
                                                   onChange={e => updateItem(idx, 'dosage', e.target.value)} 
                                                   onFocus={() => { setActiveItemIndex(idx); setSuggestionType('dosage'); setActiveTypingFieldId(`item_dosage_${idx}`); }}
                                                   onBlur={() => { setTimeout(() => { if (activeTypingFieldId === `item_dosage_${idx}`) { setSuggestionType(null); setActiveTypingFieldId(null); } }, 400); }}
                                                   onKeyDown={e => handleEnterKey(e, `item_dosage_${idx}`)}
                                               />
                                               {suggestionType === 'dosage' && activeItemIndex === idx && (
                                            ...
                                            <div className="flex-[3.5] relative overflow-visible">
                                               <input 
                                                   ...
                                                   onChange={e => updateItem(idx, 'instruction', e.target.value)} 
                                                   onFocus={() => { setActiveItemIndex(idx); setSuggestionType('instruction'); setActiveTypingFieldId(`item_instruction_${idx}`); }}
                                                   onBlur={() => { setTimeout(() => { if (activeTypingFieldId === `item_instruction_${idx}`) { setSuggestionType(null); setActiveTypingFieldId(null); } }, 400); }}
                                                   onKeyDown={e => handleEnterKey(e, `item_instruction_${idx}`)}
                                               />
                                               {suggestionType === 'instruction' && activeItemIndex === idx && (
                             ...
                             switch(el.id) {
                                case 'patientName': 
                                    ...
                                    updateFn = (v) => setSelectedPatient(prev => ({ ...(prev || {} as PatientRecord), name: toTitleCase(v) }));
                                    break;
                                case 'age': 
                                    val = selectedPatient?.age || ""; 
                                    updateFn = (v) => setSelectedPatient(prev => ({ ...(prev || {} as PatientRecord), age: v }));
                                    break;
                                case 'diagnosis': val = diagnosis; updateFn = (v) => setDiagnosis(toTitleCase(v)); break;
                                case 'chiefComplaint': val = chiefComplaint; updateFn = (v) => setChiefComplaint(toTitleCase(v)); break;
                             ...
                             const isPName = el.id === 'patientName';
                                ...
                                <div className="majestic-input-group">
                                   {isCC ? (
                                      <textarea 
                                         ...
                                         onKeyDown={e => handleEnterKey(e, el.id)}
                                         placeholder=" "
                                      ...
                                      <input 
                                         id={el.id}
                                         className={`rx-field-input w-full ${activeTypingFieldId === el.id ? 'active' : ''} ${isPatientField ? 'rx-field-input-patient' : ''} ${isPName && duplicateWarning ? 'duplicate-warning-bg' : ''}`}
                                         value={val}
                                   ...
                                   {isPName && duplicateWarning && (
                                      <div className="absolute top-[-25px] right-0 bg-red-600 text-white text-[10px] font-black px-2 py-0.5 rounded-lg flex items-center gap-1 shadow-lg animate-bounce">
                                         <AlertTriangle size={12}/> پرونده موجود است
                                      </div>
                                   )}
                                </div>
                             );
