...
const Prescription: React.FC<PrescriptionProps> = ({ initialRecord }) => {
  ...
  const handlePrint = async (mode: 'plain' | 'custom') => {
     ...
     return new Promise<void>((resolve) => {
        ...
        // ROBUST PRINTING PROTOCOL: Use Image Preloader for Custom Headers
        if (mode === 'custom' && settings.backgroundImage) {
            const img = new Image();
            img.onload = () => {
                setTimeout(() => { 
                   win.print(); 
  ...
  const handlePadReviewInConsole = () => {
    setShowDigitalPad(false);
    setViewMode('editor');
  };
      ...
      {showDigitalPad && (
        <div className="fixed inset-0 z-[420] bg-gray-950 flex flex-col overflow-hidden animate-fade-in"
           ...
           >
              ...
              <div className={`flex items-center gap-2 ${padRotation % 180 === 0 ? 'flex-col' : 'flex-row'}`}>
                 <button onPointerDown={(e) => { e.stopPropagation(); resetPadForNewPatient(); }} className="p-2 lg:p-3 text-emerald-600 bg-emerald-50 hover:bg-emerald-100 rounded-2xl transition-all" title="شروع نشست جدید">
                    <RefreshCw size={18} style={{ transform: `rotate(${-padRotation}deg)` }} />
                 </button>
                 
                 <button onPointerDown={(e) => { e.stopPropagation(); handlePadReviewInConsole(); }} className="p-2 lg:p-3 text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-2xl transition-all" title="تایید و بازبینی در کنسول">
                    <ClipboardCheck size={22} style={{ transform: `rotate(${-padRotation}deg)` }} />
                 </button>
                                ...
                                <div className="majestic-input-group">
                                   {isCC ? (
                                      <textarea 
                                         ...
                                         onKeyDown={e => handleEnterKey(el.id === 'date' ? null : e, el.id)}
                                         placeholder=" "
